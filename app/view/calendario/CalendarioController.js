Ext.define('kokojump.view.calendario.CalendarioController', {
    ...
    onClickEvento: function (obj, record, element, rowIndex, e, eOpts) {
        ...
        t = Ext.ComponentQuery.query('[name=pos]')[0];
        Ext.Ajax.request({
            url: kokojump.util.Rutas.pagosBuscar,
            params: {
                id: record.get('id')
            },
            success: function (response) {
                rs = kokojump.util.Json.decodeJSON(response.responseText);
                s = g.getStore();
                s.removeAll();
                to =0;
                if (rs) {
                    ...
                    Ext.each(rs.data, function (r, i) {
                        if(r.estado!=0){
                            ix = {
                                monto: r.monto,
                                fecha: kokojump.util.Fechas.formatFechaDB(r.fecha),    //Ext.Date.format(new Date(parseInt(fe[0]),parseInt(fe[1])-1,parseInt(fe[2])+1), 'Y-m-d') ,
                                idven: r.idven,
                                item : r.item
                            };
                            s.insert(r.item, ix);
    ...
    onClickEliminarEvento: function (b) {
        ...
        s = this.lookupReference('dgvevento');
         Ext.Msg.prompt('Seguridad', 'Ingresar la clave del administrador general !!!', function (btnx, text) {
             if (btnx == 'ok') {
                 Ext.Ajax.request({
                     url: kokojump.util.Rutas.validAdmin,
                     params: {
                         c: text.trim()
                     },
                     success: function (response) {
                         ...
                         if (rs.data[0].pasa == 0) {
                            ...
                            Ext.Ajax.request({
                                url: kokojump.util.Rutas.eventoEliminar,
                                params: {
                                    id: r.get('id')
                                },
                                success: function (response) {
                                    rx = kokojump.util.Json.decodeJSON(response.responseText);
                                    if(rx.error>0){
                                         s.getStore().reload();
                                    }
                                }
                ...
                Ext.Ajax.request({
                    ...
                    success: function (response) {
                        ...
                        if (rs.data[0].pasa == 0) {
                            ...
                            Ext.Ajax.request({
                                url: kokojump.util.Rutas.pagoAnular,
                                params: {
                                    idventa: r.get('idven')
                                },
                                success: function (response) {
    ...
    onClickImprimirContrato:function(b){
        r = b.getWidgetRecord();
        ul = kokojump.util.Rutas.imprimirContrato + r.get('id').toString();
    ...
    onClickEMail:function(b){
        r =  b.getWidgetRecord();
        Ext.Ajax.request({
            url: kokojump.util.Rutas.enviarContrato,
            params: {
                id: r.get('id')
            },
            success: function (response) {
