Ext.define('kokojump.view.producto.ProductoController', {
    ...
    onClickEliminarProducto:function(btn){
        ...
        Ext.Msg.confirm("Ventas", "Desea eliminar el SERVICIO seleccionado", function(btn){
            ...
            {
                Ext.Ajax.request({
                    url: kokojump.util.Rutas.productoEliminar,
                    params: {
                        idprod: r
                    },
    ...
    onChangeBuscarNombre : function ( txt, e, eOpts ){
        if(e.keyCode == 13){
            if(txt.getValue()!=''){
                this.lookupReference('dgvservicio').getStore().load({
                    params : { 
                        idlocal : Ext.util.Cookies.get('idlocal'),
                        nombreprod : txt.getValue()
                    }
                });
            }
