Ext.define('kokojump.view.pdv.AccionesPdv', {
    ...
    accionClienteMesa:function(btn){
        /* win = Ext.create('Ext.window.Window',{
         ...
         _win = Ext.create('kokojump.view.pdv.IngresarCliente');
        _win.show();
    },
        ...
        _store.each(function (record) {
                ...
                if (record.get('cantidad') != 0) {
                    _reg = {
                        ...
                        "numerobra": record.get("numerobra"),
                        "nino" : record.get('nino'),
                        "fono" : record.get('fono'),
                        "papa" : record.get('papa'),
                    };
         ...
         win = Ext.create('kokojump.view.pdv.Pago',{ mesa : numeromesa,persona:_idper,local:_idlocal, detalle : _jsonTexto, empleado : _idemp });
         win.show();
    },
    ...
    onSelectEmpleado: function(cbo){
         ...
         numeromesa = _btn.substr(_btn.length - 1 , _btn.length + 2);
       /* Ext.Ajax.request({
            url :kokojump.util.Rutas.mesaValidarReserva,
            params:{
              idmesa : numeromesa,
              idemp  : parseInt(cbo.getValue())
            },
            success:function(response){
                obj = kokojump.util.Json.decodeJSON(response.responseText);
    ...
    onClickVerNinosMembresia:function(){
           win = Ext.create('kokojump.view.pdv.NinoMenbresia',{});
           win.show();
    },
    onClickVerApoderados:function(){
           win = Ext.create('kokojump.view.pdv.Apoderado',{});
        win.show();
    },
     ...
     */
      onClickAgregarNino:function(btn)
      {
       ...
       numeromesa = _btn.substr(_btn.length - 1 , _btn.length + 2);
       rec = btn.getWidgetRecord();
       Ext.create('kokojump.view.pdv.Nino',{
          ...
          apo     :Ext.ComponentQuery.query('#txtNombreApoderado')[0].getValue()
