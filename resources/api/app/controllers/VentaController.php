     ...
     public function actualizargastoAction(){
           ...
           if($request->isPost() ==true)
           {
               ...
               $idlocal         = $request->getPost('idlocal');
                ...
                $format   = new FuncionesHelpers();
                $data     = array($idgasto,$fecha,$descripcion,$montogasto,$idlocal);
                //print_r($data);die();
          ...
          public function eliminargastoAction(){
           ...
           public function retirarninoAction(){
               $request        = new Phalcon\Http\Request();
               $response       = new \Phalcon\Http\Response();
               if($request->isPost() ==true)
               {
                    $data = array( 
                              $request->getPost('idven'), 
                              $request->getPost('orden')
                           );
                    $format   = new FuncionesHelpers();
                    $jsonData = Venta::retirarnino($data);
                    $response->setContentType('application/json', 'UTF-8');
                    $response->setContent(json_encode($jsonData[0], JSON_NUMERIC_CHECK));
                    return $response;
               }
