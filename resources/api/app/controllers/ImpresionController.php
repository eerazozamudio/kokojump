...
class ImpresionController extends Controller
{
    ...
    public function imprimirboletaAction($vidventa)
    {
      ...
      $pdf->setX(6.5);
      $pdf->Cell(15,3.5,"Telefonos :",$borde,0,"C");
      $pdf->setX(1.5);
      $pdf->Cell(80,3.5,"310 5569 - 950 621 806 ",0,1,"C");
      $pdf->setX(6,5);
     
    $pdf->Cell(20, 3.5, "Fecha venta : ", $borde, 0, "C");
    $pdf->setX(25,5);
    $pdf->Cell(8, 3.5, $DatosCliente["_fechaven"], $borde, 2, "L");
    $pdf->ln(1);
    $pdf->setX(6);
    $pdf->setFont("Arial", "", 7);
    ...
    foreach ($DetalleVenta as $row) {
        ...
        $pdf->setFont("Arial", "", 7);
        if($row['_hdesde'])
          $pdf->MultiCell(65, 4, $row["_producto"] .'    Tiempo : '. $row['_hdesde'].' hasta '.$row['_hhasta'], $borde, "L");
        else 
    ...
    public function exportarexcelpadreshijosAction()
    {
      ...
      foreach ($jsonData->data as $item) {
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('K'.$index, $item->direccion);
          $objPHPExcel->getActiveSheet()->setCellValue('L'.$index, $item->visitas);
          $index++;
    ...
    public function exportarexcelgastosAction()
    {
      ...
      foreach ($jsonData->data as $item) {
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('E'.$index, $item->tienda);
    ...
    public function enviarcorreoAction(){
        ...
        $pdf->Ln();
        $pdf->SetFont($font,'',$tam);
        $pdf->MultiCell(0,$in,utf8_decode("1.- 15 días antes se deberá adelantar el 50% del total del contrato el cual se cancelara al inicio del evento para la entrega del respectivo uso del local."),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("2.- Es obligatorio el uso de medias para los adultos y medias para los niños."),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("3.- Contará con el apoyo de 2 Srtas. durante el desarrollo de su evento."),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("4.- Horario: Media Hora antes para realizar decoraciones de su evento el cual se desarrollara en el tiempo previamente acordado."),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("5.- Pasado el tiempo establecido en el contrato el cliente pagara S/.500.00 por hora"),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("6.- Está prohibido el uso de pica pica"),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("7.- Esta prohibió el consumo de bebidas en ell parque"),0,'J');
        //$pdf->MultiCell(0,$in,utf8_decode("8.- Las bebidas deberán ser servidas en un vaso descartable con sorbete y tapita"),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("8.- Las mesas y sillas y activos del local son entregados en excelente estado para su uso y si algunos de ellos se dañara durante el desarrollo del evento tendrá que ser cancelado en el momento para su reposición. "),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("9.- Durante desarrollo de su evento la administración no será responsable del cuidado de artículos personales (celulares, dinero, joyas, carteras entre otros)"),0,'J');
        $pdf->MultiCell(0,$in,utf8_decode("10.- De prescindir del contrato no se devolverá el dinero adelantado, se repondrá en servicios que prestamos por KOKOJUMP"),0,'J');
        $pdf->ln();
        $fila = $pdf->getY();
